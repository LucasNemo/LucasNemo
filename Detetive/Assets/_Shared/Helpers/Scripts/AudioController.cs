...
public class AudioController : MonoBehaviour
{
    ...
    /// <param name="audio">Audio to be played</param>
    /// <param name="type">Type of sound</param>
    public void Play(AudioClip audioClip, SoundType type, float volume = 1f, bool loop = false, bool exclusiveSound = false, 
        float duration = 0, float pitch = 1f, float panStereo = 0f)
    {
            ...
            case SoundType.SoundEffect2D:
                ...
                if (exclusiveSound)
                {
                    for (int i = 0; i < _exclusiveSounds.Count; i++)
                    {
                        ...
                        {
                            _exclusiveSounds[i].Play(audioClip, loop, volume, duration, pitch, panStereo);
                            return;
                        }
                    }
                    ...
                    removeAfterPlay.Play(audioClip, loop, volume, duration,pitch,panStereo);
    ...
    private IEnumerator SwitchMusic(AudioClip audioClip, float volume)
    {
        ...
        do
        {
            _musicChannel.volume -= Time.fixedDeltaTime / 4f;
            yield return null;
        } while (Math.Round(_musicChannel.volume, 2) > 0.0f);
        ...
        do
        {
            _musicChannel.volume += Time.fixedDeltaTime * 2f;
            yield return null;
        } while (Math.Round(_musicChannel.volume, 2) < volume);
        _musicChannel.volume = volume;
        if (!_muteMusic)
        {
            _musicChannel.Play();
